...
React 18.3.1 + Vite 8 + TypeScript 6(**strict 已開啟**)+ Ant Design 6 + React Router 7 + Zustand + TanStack Query + Axios。
...
5. **元件內部依序寫:非 `react` 匯入的 hook → useRef → useState → useMemo → useEffect →
   ...
   補充(2026-09-04)。全部 24 個含 3 個以上 hook 呼叫的元件都已依此排列
   (`ThreadPanel.tsx` 是範本);這條規則沒有 lint 能強制,靠 review 把關。

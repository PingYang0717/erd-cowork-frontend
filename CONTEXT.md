...
節日前後（前 14 天到當天後 1 天）畫面上多出來的一幅場景，一個世界而不是幾塊各自的裝飾：Header 是天空與遠景（天色、遠中近三層、一個過客橫越、頭像的帽子或包住它的南瓜嘴）；天空裡落下的東西（雪、花瓣，中秋是升起的天燈）從 header 落進每個空著的面之頂端（Session 列表、還沒開始的對話、空的 Artifact 面板）；一條同一節日的地面沿著視窗底部連過 Session 列表、對話欄（從 composer 卡片後面穿過，卡片像放在地上）與空的 Artifact 面板，上面站著 header 場景裡的東西，滑過或點擊會動一下；一個節日一個角色（馴鹿、黑貓、兔子、舞獅）走完整條地面再出畫等候。空對話與空 Artifact 面板中間的 icon 磚塊換成一扇圓窗，窗裡是同一個世界的一角（同一片天氣、同一條地），住著一隻各節日自己的生物（錦鯉、知更鳥、烏鴉、月中蟾蜍），每隔一段時間從窗裡出來、穿過面板、進另一面板的窗。Session 列表另有自己的一處呼應：分隔線變成 header 那條掛串。純視覺、不帶功能，可從頭像選單一起關閉（預設開，跟主題一樣記在本機），也是畫面上唯一不出自設計稿的東西。目前四個節日：農曆新年、中秋、萬聖節、聖誕節。

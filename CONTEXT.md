...
Header 在節日前後（前 14 天到當天後 1 天）加上的裝飾層：中間一幅連續的場景帶（天色、遠中近三層）、頭像的帽子或包住它的南瓜嘴；Session 列表在同一段時間呼應它（分隔線變成同一條掛串、header 的雪或花瓣落進列表頂端、列表底部一小塊地面，滑過或點擊會動一下，並有角色偶爾走過）；空對話的畫面下方也有一小片同一節日的地面（安靜版：不會動、不理滑鼠）。純視覺、不帶功能，可從頭像選單一起關閉（預設開，跟主題一樣記在本機），也是畫面上唯一不出自設計稿的東西。目前四個節日：農曆新年、中秋、萬聖節、聖誕節。

...
## 補充(2026-09-07):第二次反問的例子換成 Lot,DC Item 已移除

本文以「SPC 掃描階段發現 **DC Item** 數量過多時第二次反問」當例子。**那個例子已不成立**:
DC Item 是 mock 時代的概念,真後端沒有,相關的 fixtures、`'dcitem'` 欄位種類、`QuestionOption`
的 `unit`/`lo`/`hi` 與 `CONTEXT.md` 詞條都已移除。

**決策本身不變**——「一次執行可以反問多次」仍然成立,而且它現在由一個真後端表達得出來的
形狀撐著:掃描到的 Lot 太多時,以純多選反問要先看哪幾個。改用真實情境的目的,是讓這條規則
的覆蓋落在真後端真的會走的路上,而不是只在 mock 的富表單裡。

同段提到「已連線的 Connector、DC Item 清單」是系統狀態的例子:Connector 那半仍然成立。

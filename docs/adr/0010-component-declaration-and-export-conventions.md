...
「不可拆」指的是**不插入別的宣告**,不是不換行:兩者型別不同,所以中間依第六點空一行。
`const streamedArtifact = state.artifact;` 與發布它的 `useEffect` 之間不會出現第三個宣告,
罩在上面的註解才有指涉對象。同理,**只有一個消費者的 `useRef` 跟著那個消費者走**——第二點裡的
...
**六、不同型別的宣告之間一律空一行**,包含第三點那種配對的內部——`const` 衍生值與它的
`useEffect` 型別不同,所以中間有一行空白。

這一條有代價,而且是刻意付的。空行同時出現在單元內與單元間,兩者權重相同,所以**單元邊界
不再由空行標記**:讀的人要靠每個單元上方那段註解去判斷哪個 `const` 配哪個 `useEffect`。
換來的是視覺上更鬆、每個宣告都有自己的呼吸空間。先前的寫法反過來——單元內不空行,一眼就
分得出四組,但整段更密。兩者都試過,這裡選後者。註解因此不只是說明,它同時是分組的標記,
刪掉它會讓配對關係無從辨認。

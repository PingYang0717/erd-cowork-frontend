...
**地帶也改成一整條。** 第一版的地帶是每個面各自把 240×40 的畫拉滿自己的寬度,三個面寬度
不同,同一道波浪在邊界就是三種刻度,接不上;而且 Artifact 面板底色是灰的,同一條雪帶在灰上很
亮、在白上幾乎看不見。現在地帶跟過客同一套:`position: fixed` 貼視窗底、寬 `100vw`,各面用
`clip-path` 裁自己那段,波浪在邊界直接接過去;地帶下面墊一層面板色(`--erd-color-bg-container`)
從地面頂端淡入,三段的地一樣亮。收合欄那條扁地帶不再需要,拿掉。

...
**同日追記:空對話也擺一片地。** 第四個上妝的位置:空對話畫面(`ThreadEmptyFestive`,標題與說明
底下)。挑它的理由是它「不是什麼」——本來就是空的、沒有任何資料要跟裝飾搶、而且只在對話開始前
看幾秒,不像 header 和側邊欄整天都在。所以它比側邊欄地面更安靜:沒有過客、沒有落下的東西、不理
滑鼠,只有靜物自己身上的光(燈籠的暈、樹上的燈)還在動。地面的圖案(色調＋各節日的地帶)這次從
`SessionRailFestive` 搬進 `festiveMotifs` 的 `GROUND_BACKDROPS`,兩邊共用同一組,不再各畫一份。
原則不變:只用空著的地方、不移動設計稿元件。

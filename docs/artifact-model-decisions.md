...
### ~~Q2 版本選單改為「此對話的產出」~~ →(2026-09-04 修訂)`vN` 恢復,但它數的不是版本

**修訂後的決議**:選單每一列前面顯示 `vN`,取自 `Artifact.version`(後端給的數字)。

**`vN` 是「這個 session 的第幾份產出」,不是「這份 Artifact 的第幾版」。** 這是產品在
Figma 上的設計。兩者是不同的概念,而後者的相關功能還沒設計(見 Q6)。

原本移除的理由是「編號讀起來像有 lineage,而那個 lineage 不存在」。那個顧慮針對的是
**衍生關係**,而這個編號說的是**產生順序**——Q1「它們是兄弟」因此仍然成立:第 2 份不是
從第 1 份長出來的,它只是第 2 個做出來的。

推導邏輯不變(帶 `artifactId` 的訊息依到達順序)。`ArtifactVersion.version` 隨之恢復,
並且由 `deriveArtifactVersions` 自己編號——一輪跑完不會重抓 artifacts 清單,只靠 join
的話剛產生的那一份會沒有號碼。

**留下的風險**:Q6 的真實版本做出來之後,「這份 Artifact 的第幾版」也會需要一個 `vN`,
和這裡的意思不同。兩者不可共用同一個徽章而不加區分——全頁檢視的選單因此**不顯示**這個
徽章(它列的是後者的位置,而手上的數字是前者)。

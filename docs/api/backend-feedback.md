...
## 中止一輪執行:半截內容留不住,也退不回去(2026-09-10)

使用者按下停止,或關掉頁面,SSE 連線就斷了。後端目前存一則固定字串的 AI 訊息
(`INTERRUPTED_TEXTS`:「回應已中斷,請重新送出以繼續」),**已經產出的內容不落檔**。

兩個後果:

**1. 半截回覆只活在前端記憶體裡。** 重新整理、切換 session、關頁面,都會讓它消失,
歷史裡只剩那句固定字串。下一輪對話 agent 也看不到自己寫到哪——因為歷史裡沒有。

希望:**中止時把已產生的內容連同中斷標記一起落檔**,而不是用固定字串取代它。前端已經
把中斷記錄當 record 呈現(灰色小字,不走 Markdown),多一則帶內容的 AI 訊息不需要新的
渲染路徑。

**2. 沒有「重試」與「編輯」,因為沒有東西刪得掉。** `GET /sessions/:id` 把 messages
內嵌回傳,寫入只有 `POST /sessions/:id/messages` 一條,而且只會往後接——**沒有獨立的
messages 端點**,不能刪、不能改、不能截斷。

所以前端只做得出**追加式**的重試:重送同一個問題,原問題與那半截回覆留在上面,agent 會
看到同一個問題兩次。「編輯訊息」則完全**沒有做**——編輯的意義是把走錯的那一段拿掉,而前端
拿不掉,硬做會變成「原問題 + 半截回覆 + 改過的問題」三者都在,那不是編輯,只是換個說法再
問一次。與其給一顆名不副實的按鈕,不如等這個能力到位。

希望:**從某則訊息之後截斷的能力**(例如 `DELETE /sessions/:id/messages?after={messageId}`)。
有了它,重試才能是「取代」而不是「再接一輪」,編輯訊息也才做得出來。

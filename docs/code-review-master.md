...
7. **資料來源 chip 是寫死的。** `10-thread-data-source-chip.md`：chip 應「reflecting the current session/Scenario's data source」。`ThreadPanel.tsx:31-34` 直接渲染字面值 `Inline DB · PT-07 line`；`types/api/session.ts` 裡也沒有任何 data-source 欄位。

...
const readPrefs = (): ConnectorPrefs => {
  try {
    ...
    if (raw !== null) {
      ...
      return { defaultConnectors: parsed.defaultConnectors ?? parsed.lastSelected ?? [] };
    }
  } catch {
...
export const writeDefaultConnectors = (connectorIds: string[]): void => {
  localStorage.setItem(CONNECTOR_PREFS_STORAGE_KEY, JSON.stringify({ defaultConnectors: connectorIds }));
};

...
describe('GET /api/hr/employeesAndOrgs', () => {
  it('finds people by NT account and org units by code', async () => {
    expect((await search('NTDEMO01')).map((entry) => entry.employeeNt)).toContain('NTDEMO01');
    expect((await search('SEC-11')).map((entry) => entry.orgId)).toContain('SEC-11');
  });
  ...
  it('answers nothing to a Chinese keyword, as the real endpoint does', async () => {
    expect(await search('示範甲')).toEqual([]);
    expect(await search('示範')).toEqual([]);
  });

...
describe('Streaming a run in the Studio', () => {
  ...
  it('shows the query results the run produced along the way', async () => {
    ...
    const table = await screen.findByRole('table', { name: 'OOC wafers on PT-01' });
    expect(within(table).getByRole('columnheader', { name: 'Wafer' })).toBeInTheDocument();
    expect(within(table).getByRole('cell', { name: 'PT-01-001' })).toBeInTheDocument();
  ...
  it('sends the answers back structured rather than as prose', async () => {
    ...
    expect(stream.requests[1]).toEqual({
      answers: { partIds: ['PT-01', 'PT-07'], timeRange: 'Last 7 days' },
  ...
  describe('the analysis conditions a Scenario asks for', () => {
    ...
    it('lets the user type a Time range the chips do not offer', async () => {
      ...
      await user.click(
        within(screen.getByRole('group', { name: 'Part ID' })).getByRole('button', { name: 'PT-01' }),
      );
      await user.click(
    ...
    it('narrows a long option list with a search box', async () => {
      ...
      expect(within(partIds).getByRole('button', { name: 'PT-07' })).toBeInTheDocument();

      await user.type(screen.getByRole('textbox', { name: '搜尋 Part ID' }), 'PT-01');

      expect(within(partIds).getByRole('button', { name: 'PT-01' })).toBeInTheDocument();
      expect(within(partIds).queryByRole('button', { name: 'PT-07' })).not.toBeInTheDocument();
    });
    ...
    it('leaves the answered conditions in the thread, collapsed', async () => {
      ...
      expect(within(values).getByText('PT-01')).toBeInTheDocument();

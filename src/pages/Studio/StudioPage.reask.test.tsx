...
describe('A reask the run is waiting on', () => {
  ...
  it('stays read-only once it has been answered', async () => {
    ...
    await user.click(chipIn(group, 'PT-02'));
  ...
  it('keeps the message when the card above could not recover it', async () => {
    ...
    historyOf(
      message({ id: 'm1', sender: 'AI', questionsJson: PART_ID_REASK }),
      message({ id: 'm2', sender: 'USER', text: 'actually, forget that — show me PT-02 yields' })
    );
    renderStudio();
    ...
    expect(await screen.findByText('actually, forget that — show me PT-02 yields')).toBeInTheDocument();
  });

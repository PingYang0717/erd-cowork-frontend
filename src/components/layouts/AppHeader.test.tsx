...
describe('AppHeader', () => {
  ...
  it('opens the account menu on the avatar, and says who is signed in', async () => {
    ...
    expect(await screen.findByText('示範甲')).toBeInTheDocument();
    expect(screen.getByText('SEC-11')).toBeInTheDocument();
  });
  ...
  it('closes on Escape and gives the focus back to the avatar', async () => {
    ...
    await screen.findByText('示範甲');

    await user.keyboard('{Escape}');

    await waitFor(() => expect(screen.queryByText('示範甲')).not.toBeInTheDocument());

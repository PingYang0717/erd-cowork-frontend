...
describe('Sharing an Artifact: picking recipients', () => {
  ...
  it('sends the typed text as `keyword`', async () => {
    ...
    await waitFor(() => expect(sent?.searchParams.get('keyword')).toBe('NTDEMO01'));
  });
  ...
  it('searches the backend once the key is long enough, and offers what it matched', async () => {
    ...
    expect(await screen.findByText(/示範甲/, {}, { timeout: 3000 })).toBeInTheDocument();
  });
  ...
  it('sends the change as add/remove, each recipient as its kind and id', async () => {
    ...
    await waitFor(() => expect(body).toEqual({ add: [{ type: 'EMPLOYEE', id: 'NTDEMO01' }], remove: [] }));
  });
  ...
  it('opens on the current recipients, and removing one sends it under remove', async () => {
    ...
    server.use(
      http.get('/api/artifacts/:id/shares', () =>
        HttpResponse.json([{ type: 'ORG', orgId: 'SEC-11', orgName: '示範一課', orgLevel: 'SECTION' }])
      ),
      http.patch('/api/artifacts/:id/shares', async ({ request }) => {
        body = await request.json();
        return HttpResponse.json([]);
    ...
    const chip = await screen.findByTitle('示範一課');
    ...
    await waitFor(() => expect(body).toEqual({ add: [], remove: [{ type: 'SECTION', id: 'SEC-11' }] }));
  });
  ...
  it('keeps a chosen recipient labelled after the search moves on', async () => {
    ...
    await user.click(field);
    await user.type(field, 'SEC-11');

    // Scoped to the chosen tags rather than the whole dialog: this search also matches
    // the same person (their org is SEC-11), so a document-wide text query would find the
  ...
  it('clears the typed text on a pick but leaves the list standing', async () => {
    ...
    await screen.findByTitle('示範甲', {}, { timeout: 3000 });
    await user.click(screen.getByTitle('示範甲'));
    ...
    expect(await screen.findByTitle('示範乙')).toBeInTheDocument();
    ...
    expect(document.querySelector('.ant-select-item-option[title="示範甲"]')).toBeNull();
  });
  ...
  it('draws a person with their photo and an organisation with its code', async () => {
    ...
    const personRow = await screen.findByTitle('示範甲', {}, { timeout: 3000 });
    ...
    expect(await screen.findByText('示範一課 (SEC-11)')).toBeInTheDocument();
  });
